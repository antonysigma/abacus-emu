...
/***/ (function(module, exports, __webpack_require__) {
	...
	function plus(j, d, type) {
	    ...
	    if (type == "show") {
	        if (d < 5) {
	            if (sum >= 10) addInstruct(plus3[d - 1]);else if (a < 5 && sum >= 5) addInstruct(plus2[d - 1]);else addInstruct(plus1[d - 1]);
	        } else if (d == 5) {
	            if (sum < 10) addInstruct(plus1[d - 1]);else addInstruct(plus3[d - 1]);
	        } else {
	            if (sum < 10) addInstruct(plus1[d - 1]);else if (a >= 5 && sum < 15) addInstruct(plus4[d - 6]);else addInstruct(plus3[d - 1]);
	        }
	        //add operation to queue
	        if (sum >= 10) instructObj.queue(function () {
	            setNumber(j, sum - 10);
	            if (j - 1 >= 1) plus(j - 1, 1);else overflow();
	            $(this).dequeue();
	...
	function minus(j, d, type) {
	    ...
	    if (type == "show") {
	        if (d < 5) {
	            if (diff < 10) addInstruct(minus3[d - 1]);else if (a >= 5 && diff < 15) addInstruct(minus2[d - 1]);else addInstruct(minus1[d - 1]);
	        } else if (d == 5) {
	            if (diff >= 10) addInstruct(minus1[d - 1]);else addInstruct(minus3[d - 1]);
	        } else {
	            if (diff >= 10) addInstruct(minus1[d - 1]);else if (a < 5 && diff >= 5) addInstruct(minus4[d - 6]);else addInstruct(minus3[d - 1]);
	        }
	        //add operation to queue
	        if (diff < 10) instructObj.queue(function () {
	            setNumber(j, diff);
	            if (j - 1 >= 1) minus(j - 1, 1);else {
	                overflow();
	...
	function times(j, a, d, type, flag_replace) {
	    ...
	    if (type == "show") {
	        if (a > d) addInstruct(times1[a - 1][d - 1]);else addInstruct(times1[d - 1][a - 1]);
	
	        //add operation to queue
	        instructObj.queue(function () {
	            if (j + 1 <= digits) {
	                if (flag_replace) //overwrite the original digit
	                    setNumber(j, Math.floor(prod / 10));else plus(j, Math.floor(prod / 10));
	                plus(j + 1, prod % 10);
	            } else overflow();
	            $(this).dequeue();
	        });
	...
	divide1[0] = ['逢一進一 1/1=1', '逢二進二 2/2=2', '逢三進三 3/3=3', '逢四進四 4/4=4', '逢五進五 5/5=5', '逢六進六 6/6=6', '逢七進七 7/7=7', '逢八進八 8/8=8', '逢九進九 9/9=9'];
	...
	var divide3 = ['見一無除作九一', '見二無除作九二', '見三無除作九三', '見四無除作九四', '見五無除作九五', '見六無除作九六', '見七無除作九七'];
	var divide4 = ['借一還一', '借一還二', '借一還三', '借一還四', '借一還五', '借一還六', '借一還七', '借一還八', '借一還九'];
	...
	function divide_by(j, a, b) {
	    ...
	    if (true_quo > quo) {
	        ...
	        if (quo_diff * d <= 9) addInstruct(divide1[d - 1][quo_diff - 1]);else addInstruct('Repeat "' + divide1[d - 1][0] + '" by ' + quo_diff + ' times');
	        instructObj.queue(function () {
	        ...
	        quo_diff *= -1;
	        if (quo_diff > 1) addInstruct('Repeat "' + divide4[d - 1] + '" by ' + quo_diff + ' times');else addInstruct(divide4[d - 1]);
	    ...
	    if (true_quo > 0 && Math.log10(b - d / 10) > -digits) {
	        var y = Math.round((b - d / 10) * Math.pow(10, digits)) / Math.pow(10, digits - 1);
	        ...
	        var _loop = function _loop(k) {
	            var r = Math.floor(y);
	            if (r > 0) instructObj.queue(function () {
	                minus(k, r);
	                $(this).dequeue();
	            });
	            y = (y - r) * 10;
	...
	$(function () {
	    $('button:contains("Reset")').click(function () {
	        ...
	        instructObj.html('');
	        for (var j = 1; j <= digits; j++) {
	            //from 3nd column to 11th column
	            setNumber(j, 0);
	        }
	    });
	});
